...
class DateDataloader(DataLoader):
    ...
    def __init__(self, config_data, use_cache=False):
        super().__init__(config_data, use_cache=use_cache)
    ...
    def __next__(self):
        if self.current_index < self.data_length:
            fake_samples = [self.fake_dataset[self.liste_dates_rep[self.current_index + i], self.current_index + i] for i in range(self.batch_size)]
            obs_samples = [self.obs_dataset[self.liste_dates_rep[self.current_index + i], self.current_index + i] for i in range(self.batch_size)]
            real_samples = [self.real_dataset[self.liste_dates_rep[self.current_index + i], self.current_index + i, self.Lead_Times, self.dh, self.df0] for i in range(self.batch_size)]
