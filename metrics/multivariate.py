...
def plot2D_histo(var2var_f, var2var_r, levels, output_dir, add_name):
    ...
    for i in range(ncouples):
       cs=axs[i].contourf(bins_r[Xsindices[i]][:-1], bins_r[Ysindices[i]][:-1],bivariates_r[i], cmap='plasma', levels=levels[i])
       axs[i].contour(bins_r[Xsindices[i]][:-1], bins_r[Ysindices[i]][:-1], bivariates_f[i],cmap='Greys', levels=levels[i])
...
def var2Var_hist(data, bins, density=True):
    ...
    for tu in var_couples:
        ...
        if i!=j:
            if type(bins)==int:
                bivariates[k], Bins[i], Bins[j]=\
                                np.histogram2d(data[:,i], data[:,j], bins=bins, density=density)
            elif type(bins)==np.ndarray:
                bivariates[k], _, _=\
                                np.histogram2d(data[:,i], data[:,j], bins=[Bins[i], Bins[j]], density=density)
            k+=1
