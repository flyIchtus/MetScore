...
def skill_spread_deviation_multi_dates(cond, X, real_ens, debiasing = False, print_skill_aro=False):
    ...
    if debiasing:
        ...
        print(X_p[:,:,2].mean(), real_ens[:,:,2].mean())
        ...
        if print_skill_aro:
            ...
            print('skill aro', np.nanmean(np.sqrt(np.nanmean(skill_aro, axis=0)), axis=(-2,-1)))
    ...
    print('spread',spread_aggregated, 'skill', skill_aggregated, flush=True)

    deviation = np.abs(skill_aggregated - spread_aggregated)

    return deviation

def thresholded_skill_spread_deviation_multi_dates(cond, X, real_ens, debiasing = False, print_skill_aro=False, threshold=0.8):
    """
    ...
    Inputs :
        ...
        threshold :  the scale at which skill must match spread --> if 0.5, means deviation will be 0 when spread is half the skill
        
    Returns :
        
        deviation :  C x H x W array containing the result 
    ...
    deviation = np.abs(threshold * skill_aggregated - spread_aggregated)
...
def skill_spread_vs_aro_multi_dates(cond, X, real_ens, debiasing = False):
    ...
    print('spread',spread_aggregated, 'skill', skill_aggregated, flush=True)
    print('spread aro',spread_aggregated_aro, 'skill aro', skill_aggregated_aro, flush=True)
